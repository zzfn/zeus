    ...
    const columns = [
        {
            title: '仓库',
            dataIndex: 'slug',
            render: (text: string, record: any) => <a target="_blank" href={record.link}>{text}</a>
        },
        {
            title: '最近更新时间',
            render: (_text: string, record: any) => <span>{dayjs.duration(dayjs.unix(record.build?.finished).diff(dayjs())).humanize(true)}</span>
        },
        {
            title: '操作',
            render: (_: string, record: any) => <Link to={`/drone/${record.namespace}/${record.name}`}>详情</Link>
        },
    ];
